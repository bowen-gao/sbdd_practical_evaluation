...
from models.fusion_modules import CrossAttentionFusionModules,Concatenation
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        # if config.fusion do not exist
        if not hasattr(config, 'fusion'):
            print("#"*60) 
            print("\"fusion\" config does not exist in ckpt, set to False")
            print("#"*60)
            config.fusion = False
        if config.fusion:
            if "xattn" in config.fusion_type:
                self.fusion_model = CrossAttentionFusionModules(
                    config= config,
                    fusion_type=config.fusion_type,
                    fusion_pos=config.fusion_pos,
                    h_dim=config.hidden_dim,
                    ret_feat_dim=config.ret_feat_dim,
                    n_heads=config.n_heads,
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
                time_step=None, return_all=False, fix_x=False,ret_feat=None):
        ...
        if self.config.fusion:
            h_all=self.fusion_model(h_all,ret_feat,batch_all,mask_ligand,"init")


        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, return_all=return_all, fix_x=fix_x,ret_feat=ret_feat,fusion_model=self.fusion_model)
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step=None,ret_feat=None
    ):
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         ...
                         num_steps=None, center_pos_mode=None, pos_only=False,ret_feat=None):
