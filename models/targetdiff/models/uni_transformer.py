...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self,num_layers, l_idx, h, x, batch, edge_attr, edge_index, mask_ligand, e_w=None, fix_x=False, fusion_model=None,ret_feat=None):
        src, dst = edge_index
        ...
        if fusion_model is not None and l_idx == num_layers - 1:
            new_h=fusion_model(new_h,ret_feat,batch,mask_ligand,"ending")
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False,ret_feat=None,fusion_model=None):
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, x = layer(self.num_layers, l_idx, h, x, batch, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x, fusion_model=fusion_model,ret_feat=ret_feat)
                if fusion_model is not None and l_idx != self.num_layers - 1:
                    h=fusion_model(h,ret_feat,batch,mask_ligand,l_idx)
            all_x.append(x)
            all_h.append(h)
