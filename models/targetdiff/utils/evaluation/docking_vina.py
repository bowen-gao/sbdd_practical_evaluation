...
class VinaDockingTask(BaseDockingTask):
    ...
    def from_original_data(cls, data, ligand_root='./data/crossdocked_pocket10', protein_root='./data/crossdocked',
        ...
        # )
        protein_name= data.ligand_filename.split('/')[0]
        protein_path = os.path.join(protein_root, protein_name, 'recepter.pdb')
    ...
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, protein_root='./data/crossdocked', **kwargs):
        ...
        # for dude
        # protein_name= ligand_filename.split('/')[0]
        # protein_path = os.path.join(protein_root, protein_name, 'receptor.pdb')


        # for pcba
        protein_name = ligand_filename.replace("_ligand.mol2", "_protein.pdb")
        protein_path = os.path.join(protein_root, protein_name,)

        print("protein_path for vina", protein_path)
