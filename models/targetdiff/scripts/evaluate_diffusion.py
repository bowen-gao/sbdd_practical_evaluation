...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_num_examples', type=int, default=None, help='Number of examples to evaluate, use for debug')
    parser.add_argument('--save', type=eval, default=True)
    parser.add_argument('--protein_root', type=str, default='/data/DUD-E/raw')
    ...
    results_fn_list = sorted(results_fn_list, key=lambda x: int(os.path.basename(x)[:-3].split('_')[0]))
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        print("evaluating", r_name)
        case_name=os.path.basename(r_name).split('.')[0]
        ...
        for sample_idx, (pred_pos, pred_v) in enumerate(zip(all_pred_ligand_pos, all_pred_ligand_v)):
            ...
            try:
                ...
                if args.save_as_sdf:
                    if mol is not None:
                        mol.SetProp('_Name', f'{case_name}_{sample_idx}')
        ...
        if args.save_as_sdf:
            w = Chem.SDWriter(os.path.join(args.sample_path, f'{case_name}.sdf'))
