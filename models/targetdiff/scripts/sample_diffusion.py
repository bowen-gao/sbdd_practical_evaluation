...
def sample_diffusion_ligand(ckpt_config,model, data, num_samples, batch_size=16, device='cuda:0',
    ...
    for i in tqdm(range(num_batch)):
        ...
        with torch.no_grad():
            ...
            # ret_feat
            if ckpt_config.model.fusion:
                ret_feat=getattr(data, ckpt_config.model.ret_feat_version, None)
                ret_feat= torch.tensor(np.array(ret_feat)).to(args.device)
                ret_feat=ret_feat.unsqueeze(0).repeat(n_data,1,1)
            else :
                ret_feat=None
...
def align_model_state_dict(state_dict):
    # insert .cross_attention to the parameters name
    new_state_dict = {}
    for k, v in state_dict.items():
        para_name_list=k.split(".")
        if para_name_list[0] == "fusion_model":
            if para_name_list[1] == "cross_attention_modules":
                if isdigit(para_name_list[2]):
                    para_name_list.insert(3, "cross_attention")
        new_state_dict[".".join(para_name_list)] = v
...
if __name__ == '__main__':
    ...
    # res_path=os.path.join(args.result_path, f'result_{args.data_id}.pt')
    # if os.path.exists(res_path):
    #     print(f'{res_path} already exists, skip!')
    #     exit(0)
    ...
    # ckpt['model'] = align_model_state_dict(ckpt['model'])
    # print different parameters
    name_only_in_model=[]
    name_only_in_ckpt=[]
    for name, param in model.named_parameters():
        if name not in ckpt['model']:
            name_only_in_model.append(name)
    for name, param in ckpt['model'].items():
        if name not in model.state_dict():
            name_only_in_ckpt.append(name)
    print("name_only_in_model:",name_only_in_model)
    print("name_only_in_ckpt:",name_only_in_ckpt)
    ...
    torch.save(result, os.path.join(result_path, f'{args.data_id}_{target_name}.pt'))
