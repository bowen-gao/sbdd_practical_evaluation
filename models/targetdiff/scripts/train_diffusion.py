...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='/data/targetdiff_data/logs_diffusion')
    ...
    if args.wandb is not None:
        wandb.init(project='benchmark_targetdiff', config=config, dir=log_dir, name=args.wandb)
    ...
    def train(it):
        ...
        for _ in range(config.train.n_acc_batch):
            ...
            if config.model.fusion:
                ret_feat=getattr(batch, config.model.ret_feat_version, None)
        ...
        # wandb
        if args.wandb is not None:
            wandb.log({'train/loss': loss, 'train/loss_pos': loss_pos, 'train/loss_v': loss_v, 'train/grad': orig_grad_norm})
    ...
    def validate(it):
        ...
        # wandb
        if args.wandb is not None:
            wandb.log({'val/loss': avg_loss, 'val/loss_pos': avg_loss_pos, 'val/loss_v': avg_loss_v, 'val/atom_auroc': atom_auroc})
        ...
        for it in range(1, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                if best_loss is None or val_loss < best_loss:
                    ...
                    # save to /nfs/data/targetdiff_data/ckpts
                    if args.wandb is not None:
                        ckpt_path = os.path.join('/data/targetdiff_data/ckpts', args.wandb+'.pt')
                        torch.save({
                            'config': config,
                            'model': model.state_dict(),
                            'optimizer': optimizer.state_dict(),
