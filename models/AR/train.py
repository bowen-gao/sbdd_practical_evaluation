...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='/data/AR_data/logs')
    ...
    if args.wandb is not None:
        wandb.init(project='benchmark_AR', config=config, dir=log_dir, name=args.wandb)
        ...
        if args.wandb is not None:
            if it%10==0:
                wandb.log({
                    ...
                    'train/loss_ind': loss_ind.item(),
                    'train/loss_real': loss_nce_real.item(),
                    'train/loss_fake': loss_nce_fake.item(),
        ...
        if args.wandb is not None:
            if it%10==0:
                wandb.log({
                    ...
                    'val/loss_ind': loss_ind.item(),
                    'val/loss_real': loss_nce_real.item(),
                    'val/loss_fake': loss_nce_fake.item(),
                }, step=it)
        ...
        for it in range(1, config.train.max_iters+1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                if best_loss is None or val_loss < best_loss:
                    ...
                    # save to /data/AR_data/ckpts
                    if args.wandb is not None:
                        ckpt_path= f'/data/AR_data/ckpts/{args.wandb}.pt'
                        torch.save({
                            'config': config,
                            'model': model.state_dict(),
                            'optimizer': optimizer.state_dict(),
