...
if __name__ == '__main__':
    ...
    parser.add_argument('--outdir', type=str, default='/data/pocket2mol_data/sample_output/XXX')
    parser.add_argument('--device', type=str, required=True)
    parser.add_argument('-i', '--data_id', type=str, required=True)
    ...
    name=str(data_id)+"_"+base_data['protein_filename'].split("/")[0]
    print("Process case:",name)

    # # Logging
    # log_dir = get_new_log_dir(args.outdir,name)
    ...
    # # Save sdf mols, write all mol to sdf_all_dir
    sdf_dir = os.path.join(log_dir, 'SDF')
    sdf_all_dir = os.path.join(log_dir,"generated_mols.sdf")
    writer=Chem.SDWriter(sdf_all_dir)
    os.makedirs(sdf_dir)
