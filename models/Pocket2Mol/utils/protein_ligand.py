        ...
        for line in self.block.splitlines():
            if line[0:6].strip() == 'ATOM':
                ...
                yield {
                    ...
                    'y': float(line[38:46]),
                    'z': float(line[46:54]),
                    'occupancy': float(line[54:60]) if line[54:60].strip()!='' else 1.00,
...
def parse_sdf_file(path):
    ...
    if rdmol is None:
        raise ValueError("Invalid mol file: %s" % path)
    frags = Chem.GetMolFrags(rdmol)
    assert len(frags) == 1 , "Molecule have more than one fragment"
