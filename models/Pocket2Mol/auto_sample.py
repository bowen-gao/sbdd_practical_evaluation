...
class AutoExpRunner:
    def __init__(self,args):
        ...
        self.ckpt_path=os.path.join("/data/pocket2mol_data/ckpts",self.exp_name+".pt")
        if args.save_dir_basename is not None:
            self.sample_output_path=os.path.join("/data/pocket2mol_data/PCBA_sample_output",args.save_dir_basename)
        else:
            self.sample_output_path=os.path.join("/data/pocket2mol_data/PCBA_sample_output",self.exp_name)
    ...
    def _sample_worker(self, gpu_id, tasks):
        for task in tasks:
            ...
            print("#"*50)
            cmd = f"taskset -c {task} python sample.py --ckpt {self.ckpt_path} --data_id {task} --device cuda:{gpu_id} --outdir {self.sample_output_path}"
            os.system(cmd)
