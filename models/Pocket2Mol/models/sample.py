...
def add_ligand_atom_to_data(data, pos, element, bond_index, bond_type, type_map=[6,7,8,9,15,16,17]):
    ...
    # Debug
    # bacause the type_map inconsistency with the element type, this is a temporary solution
    assert len(type_map) == 7
    try:
        one_hot_tensor=F.one_hot(element.view(1), len(type_map))
    except:
        one_hot_tensor=torch.zeros(1, len(type_map))

    data.ligand_context_feature_full = torch.cat([
        data.ligand_context_feature_full,
        torch.cat([
            one_hot_tensor.to(data.ligand_context_feature_full), # (1, num_elements)
    ...
    #Debug
    # because the type_map inconsistency with the element type, this is a temporary solution
    try:
        element = torch.LongTensor([type_map[element.item()]])
    except:
        element = torch.LongTensor([35])
