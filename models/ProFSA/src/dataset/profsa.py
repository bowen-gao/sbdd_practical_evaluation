...
def process_mol(
    ...
    atoms = torch.from_numpy(mol_dict.vec_index(atoms)).long()

    # prepend and append atoms
    atoms = torch.cat(
        [torch.full_like(atoms[0], mol_dict.bos()).unsqueeze(0), atoms], dim=0
    )
    atoms = torch.cat(
        [atoms, torch.full_like(atoms[0], mol_dict.eos()).unsqueeze(0)], dim=0
    )

    # edge type
    edge_type = atoms.view(-1, 1) * len(mol_dict) + atoms.view(1, -1)
