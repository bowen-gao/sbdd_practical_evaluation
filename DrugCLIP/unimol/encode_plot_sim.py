...
def main(args):
    ...
    for path in test_path_list:
        ...
        for target in tqdm(targets):
            ...
            import numpy as np
            sim = np.dot(generate_reps, fda_reps.T)
        ...
        method_sim_list.append(list(sim_list)) 
    ...
    ef_data = {
        "LiGAN": method_sim_list[0],
        "AR": method_sim_list[1],
        "Pocket2Mol": method_sim_list[2],
        "TargetDiff": method_sim_list[3],
        "MolCRAFT": method_sim_list[4]
    }
    ...
    df_ef = pd.DataFrame(ef_list, columns=["Method", "Max Similarity"])
    ...
    df_ef_filtered = df_ef.groupby("Method").apply(lambda x: remove_outliers(x, "Max Similarity")).reset_index(drop=True)
    ...
    print(df_ef_filtered["Max Similarity"].min())

    # Step 4: Generate Violin Plot
    plt.figure(figsize=(10, 6))
    sns.violinplot(x="Method", y="Max Similarity", data=df_ef_filtered)
    #plt.ylim(0, 20)
    ...
    plt.ylabel("Max Similarity", fontsize=20)
    plt.grid(True)
    ...
    plt.savefig("violin_plot_fda_unimol.png", dpi = 300, bbox_inches = 'tight')
