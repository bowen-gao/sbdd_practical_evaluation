...
class DrugCLIP(UnicoreTask):
    ...
    def encode_mols_once(self, model, data_path, emb_dir, atoms, coords, **kwargs):
        ...
        if os.path.exists(cache_path):
            with open(cache_path, "rb") as f:
                mol_reps, mol_names, mol_labels = pickle.load(f)
            return mol_reps, mol_names, mol_labels
        ...
        for _, sample in enumerate(tqdm(mol_data)):
            ...
            mol_reps.append(mol_emb)
            mol_names.extend(sample["smi_name"])
            mol_labels.append(sample["target"].detach().cpu().numpy())  


        mol_reps = np.concatenate(mol_reps, axis=0)
        mol_labels = np.concatenate(mol_labels, axis=0)

        # save the results
        
        with open(cache_path, "wb") as f:
            pickle.dump([mol_reps, mol_names, mol_labels], f)

        return mol_reps, mol_names, mol_labels
    
    
    def encode_unimol_mols_once(self, model, data_path, emb_dir, atoms, coords, **kwargs):
    ...
    def encode_pocket_once(self, model, pocket_path, **kwargs):
